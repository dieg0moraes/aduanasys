-- ============================================
-- Migrar columnas de embedding de VECTOR(1536) a VECTOR(384)
-- para usar modelo local multilingual-e5-small
-- Ejecutar en el SQL Editor de Supabase
-- ============================================

-- 1. Borrar índices vectoriales existentes (usan dimensión 1536)
DROP INDEX IF EXISTS idx_catalog_embedding;
DROP INDEX IF EXISTS idx_ncm_embedding;

-- 2. Borrar funciones que referencian VECTOR(1536)
DROP FUNCTION IF EXISTS search_product_catalog;
DROP FUNCTION IF EXISTS search_ncm;

-- 3. Cambiar columnas de embedding a 384 dimensiones
ALTER TABLE product_catalog ALTER COLUMN embedding TYPE VECTOR(384);
ALTER TABLE ncm_nomenclator ALTER COLUMN embedding TYPE VECTOR(384);

-- 4. Recrear índices vectoriales con 384 dimensiones
...
-- 5. Recrear función de búsqueda en catálogo
...
-- 6. Recrear función de búsqueda en NCM
